...
int barcode_record[MAX_RECORDS];
int compartment_record[MAX_RECORDS];
int oldest = 0;
int ptr = 0;
...
int storeBarcode(int b_data, int m_data) {
	...
	struct sensorInfo *new_node = (struct sensorInfo*) malloc(sizeof(struct sensorInfo));
	...
	compartment_record[ptr] = maskBarcode(b_data);
...
int removeBarcode(void) {
	...
	while(tracking_node -> next != old) { //loop to the 2nd oldest node of linked list
	...
	//=======================
	/**
	compartment_record[oldest] = 0;
	barcode_record[oldest] = 0;
...
void choose_compartment_pwm(int compart_num) {
	...
	int compartment_num = compart_num;
	...
	micro_wait(2000);
	//pwm(1,0);  //return servo arms to default position
	//pwm(2,0);
	//pwm(3,0);
	/**
	TIM2->CCR1 = 90;
...
int main(void)
{
	...
	while(1)
	{
		...
		if((tmp == '\r') || (tmp == '\n'))  //checks for newline or carriage return
		{
			i = 0;
			//get_msg_flag = 1;
			//printf("Barcode = %s",buf);  //REPLACE WITH TERA TERM INSTANCE!
			//barcode = (uint8_t) atoi(buf);  //convert char buf into uint8_t  (<= not sure if this is what it should be doing)
			...
			choose_compartment_pwm(temp_compartment);

			//choose_compartment_pwm(temp_compartment);
			/**
			 * if (tof_interrupt) {
			 * 		interrupt flag = 1 (invokes PWM)
			 *
			 * }
			 */
			//interupt distance ISR here (raise flag?)
			//then clear buf for next scan
			//temp_compartment = maskBarcode(barcode);
			//choose_compartment_pwm(temp_compartment);
