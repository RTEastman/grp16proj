...
void setup_gpio() {
	...
	GPIOA->MODER &= ~((3<<(0*2))+(3<<(1*2))+(3<<(2*2))+(3<<(3*2)));    //clear the bits first
	GPIOA->MODER |= ((2<<(3*2))+(2<<(0*2))+(2<<(1*2))+(2<<(2*2)));    //sets pins to alternative function mode (10 aka 2) (PA0-3 will need this)
	GPIOA->AFR[0] |= ((2<<(0*4))+(2<<(1*4))+(2<<(2*4))+ (2<<(3*4)));    //set to AF2 (PA0-3 need AF2 aswell for TIM2, probably with AFR[0] instead of AFR[1])
...
void setup_pwm() {
    ...
    TIM2->CCMR2 |= TIM_CCMR2_OC4M_2 + TIM_CCMR2_OC4M_1;	//Output compare 3 mode (PWM mode 1 for channel 3?)

    TIM2->CCER |= ((TIM_CCER_CC1E)+(TIM_CCER_CC2E)+(TIM_CCER_CC3E)+(TIM_CCER_CC4E)); //OCx output to specified pins enabled
...
int pwm(int device_no, int deg_no)
{

	//setup_gpio();  //moved to main.c
	//setup_pwm();  //moved to main.c
	...
	/**while(1){
		...
		if(cw == 1){
			...
			cnt++;  //literally just wait 1000 ms
		}
		else if(ccw == 1){
			if(cnt == cntSet){
				deg -= 1;
